...
class Guess:
    ...
    def play(self):
        global choice
        choice = [str(i)+'.ogg' for i in range(1,11)]
        try:
    ...
    def check(self,anime_name,character_name):
        ...
        if len(value) == 0 or len(char) == 0:
            mb.showerror("Try Again","Yo enter Something")
        elif value in anime_name and character_name in char:
    ...
    def main(self):
        global guess_anime,guess_character
        pygame.init()
        pygame.mixer.init()
        root = tk.Tk()
        ...
        play_dialouge = tk.Button(text="Play Me",font=("arial",14,"bold"),bd=5,command=self.play,bg="purple",fg="aqua").place(x=190,y=100,width=100,height=45)
        ...
        check = tk.Button(text="Check Result",font=("arial",12,"bold"),bd=5,command=self.result,bg="purple",fg="aqua").place(x=320,y=320,width=120,height=50)
        root.mainloop()
